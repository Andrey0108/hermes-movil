...
CREATE TABLE
    usuario (
        ...
        CONSTRAINT CHK_tipoDocumento CHECK (
            tipoDocumento IN (
                'CC',
                'CE',
                'PA',
...
CREATE TABLE
    cliente (
        ...
        CONSTRAINT CHK_tipoDeSangre CHECK (
            tipoDeSangre IN ('A+', 'A-', 'B+', 'B-', 'AB+', 'AB-', 'O+', 'O-')
        ),
    );
...
CREATE TABLE
    reserva (
        ...
        CONSTRAINT CHK_estadoReservaReservation CHECK (
            estadoReserva IN ('N', 'C', 'P', 'M', 'R', 'A', 'E', 'F')
        )
    );
